...
# original SDE-MOEA + extreme perserve + uniformity
def SDE_parent_selection3(popobjs):
    ...
    for i in range(N):
        for j in range(N):
            ...
            if i == j:
                ...
                SDE_Mat[i, j] *= np.linalg.norm(popobjs[i, :]-popobjs[j, :], ord=2, axis=0)
...
def SDE_env_selection3(popobjs, n):
    # popobjs (N * M): objectives of population, #poluation = n, #objectives = m
